...

{
  ...
  config = mkIf config.cistern.noisefs.enable {
    ...
    # System packages and NoiseFS management utilities
    environment.systemPackages = with pkgs; [
      ...
      # NoiseFS management utilities
      (writeShellScriptBin "cistern-noisefs-swarm" ''
        #!/usr/bin/env bash
      ...
      '')
      (writeShellScriptBin "cistern-noisefs" ''
        #!/usr/bin/env bash
        ...
        case "''${1:-help}" in
          status)
            ...
            echo ""
            echo "=== IPFS Peers ==="
            ${curl}/bin/curl -s -X POST http://127.0.0.1:${toString config.cistern.noisefs.ipfs.apiPort}/api/v0/swarm/peers | ${jq}/bin/jq '.Peers[] | .Addr' 2>/dev/null || echo "No peers connected"
            echo ""
            echo "=== Mount Status ==="
